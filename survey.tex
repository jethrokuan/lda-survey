...
LDA trained in 65 seconds, and the full results are shown in the
...
To train DTM, we split the documents into slices corresponding to
their years. DTM took 3.9 hours to train, substantially longer than
LDA, and this can be attributed to the increased model complexity. We
inferred the topic distributions for three time slices, and they are
displayed in the appendix (~\autoref{fig:dtm_slice1},
~\autoref{fig:dtm_slice2}, ~\autoref{fig:dtm_slice3}).
...
We notice that the highly probable words in each topic remain largely
unchanged across each time slice. ~\autoref{fig:dtm_trunc}} shows that
in the last time slice, ``markov'' managed to squeeze into the top 6
words of the abstract topic. Perhaps this is indicative of an
increased usage of Markov related techniques in the later years.
...
We look at the analysis of results from each of the papers. In the
seminal paper for LDA, \citeauthor{blei2003latent} computed the
